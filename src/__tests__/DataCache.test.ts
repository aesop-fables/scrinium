...
interface SubscriptionProxy<Compartments> {
  <T>(name: keyof Compartments): Promise<T>;
}

interface DataCacheScenario<Compartments> {
...
function createDataCacheScenario<Compartments extends Record<string, any>>(
  policy: Compartments,
